...
export const updateSalesSummary = async (data: any) => {
  ...
  return prisma.salesSummaryDaily.upsert({
    ...
    update: {
      total_revenue: { increment: new Prisma.Decimal(data.revenue) },
      total_tax: { increment: new Prisma.Decimal(data.tax) },
      total_transactions: { increment: 1 }
    },
    create: {
      summary_date: date,
      total_revenue: new Prisma.Decimal(data.revenue),
      total_tax: new Prisma.Decimal(data.tax),
...
export const updateInventory = async (data: any) => {
  return prisma.inventorySummary.upsert({
    ...
    update: {
      ...
      stock_value: new Prisma.Decimal(data.stock_value)
    },
    create: {
      product_id: data.product_id,
      branch_id: data.branch_id,
...
export const updatePayroll = async (data: any) => {
  return prisma.payrollSummary.upsert({
    ...
    update: {
      total_payroll: { increment: new Prisma.Decimal(data.amount) }
    },
    create: {
      payroll_month: data.month,
      payroll_year: data.year,
      total_payroll: new Prisma.Decimal(data.amount)
    }
  });
};
...
export const updateSupplier = async (data: any) => {
  return prisma.supplierSummary.upsert({
    ...
    update: {
      total_purchase_value: { increment: new Prisma.Decimal(data.amount) }
    },
    create: {
      supplier_id: data.supplier_id,
      branch_id: data.branch_id,
      total_purchase_value: new Prisma.Decimal(data.amount)
    }
  });
};
